...
Role dev destina-se a desenvolvedores que precisam acessar ferramentas técnicas do ecossistema CARF. Diferente das roles operacionais que seguem hierarquia em árvore (super-admin > admin > manager > [analyst | field-coordinator > field-cadastrator]), a role dev é transversal e pode ser combinada com qualquer outra role sem relação de herança.
...
A role dev ainda não está presente no realm-export.json e deve ser adicionada antes do primeiro deploy. No Keycloak será realm role simples sem composite roles associadas. Deve ser atribuída manualmente a usuários da equipe de desenvolvimento. Token JWT inclui dev no array de roles quando presente permitindo middleware do WebDocs validar acesso às rotas protegidas.
