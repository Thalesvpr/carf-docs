...
Logout chama keycloak.logout({ redirectUri: window.location.origin }) que faz GET ao /protocol/openid-connect/logout invalidando sessão SSO fazendo logout de todas as 6 aplicações CARF simultaneamente (REURBWEB, REURBCAD, GEOAPI, GEOGIS, WEBDOCS, ADMIN) e redirect de volta para origin, token usage em API calls via Axios interceptor que antes de cada request chama await keycloak.updateToken(5) garantindo token válido e adiciona `Authorization: Bearer ${keycloak.token}` no header, se backend retorna 401 com "Token is not active" interceptor response detecta e chama keycloak.login() forçando re-autenticação.
